# ADR-52: DuetsPad Modal Dialog Surface
...
A browser dialog cannot synchronously return a future user choice to the script
...
represent the dialog itself: control events are transient commands, while an
open dialog must remain authoritative across SSE reconnects and must own the
...
- Reuse `DisplayContent`, interaction invocation, field snapshots, attachment
  ...
  mechanisms.
- Make open-dialog state reconnectable and give every retained handler, field,
  slot, and attachment an explicit lifetime.
- Execute a dialog result callback at most once under duplicate clicks and
...
- Bound long-lived dialog state independently of authentication.

## Considered Alternatives

### A: Return or await the user's choice in the opening turn

- Pro: resembles native `prompt` and Promise-based browser dialog APIs.
...
### B: Send `control.dialog.open` and `control.dialog.close` commands

- Pro: extends the existing imperative command serializer with two operations.
- Con: an open dialog is durable server-owned state that must be replayed and
...
### C: Add a server-canonical, callback-driven Dialog surface (chosen)

- Pro: fits the existing turn model, reuses rendering and interaction transport,
  makes reconnect behavior explicit, and gives dialog-owned resources a precise
  lifetime.
...
- Pro: avoids a dialog patch protocol and some placement work.
...
`ui.dialog` follows the callback-first shape of `ui.button`:
...
automatically. Ordinary interactions inside the body do not close the dialog and
...
Each session owns an insertion-ordered active-dialog collection under
`_stateLock`. A dialog projection contains its id, revision, reduced body tree,
committed body and footer interactions, and presentation options. Its committed
footer and dismiss handlers retain the result callback while the dialog is
active. The session limit `MaxActiveDialogs` defaults to 8; `null` means
unlimited, and opening beyond the limit fails before committing resources.

`InteractionStore` owns committed interactions by dialog id in addition to its
Canvas and Timeline collections. Closing a dialog unregisters every body,
footer, and dismiss handler immediately. Field, slot, and attachment placement
searches include every active dialog tree. Script-originated field writes,
browser commits, slot updates, attachment projection, pruning, and invoke
validation all use those dialog roots.

A footer or dismiss handler applies the invoke snapshot, then atomically claims
the still-open dialog before calling user code. The first claim wins; subsequent
requests receive the existing stale-interaction outcome. The callback runs while
the dialog's fields and attachments remain reachable. A `finally` boundary then
closes the dialog, releases its interactions, prunes field-backed state, and
broadcasts the close even when user code throws. Session reset and disposal
discard dialogs without invoking callbacks.

### Projection protocol and reconnect

Dialogs are a state namespace on the existing multiplexed SSE stream, not a
`control.*` command:

- `dialog.snapshot`: the ordered full set of active dialog projections for a
  newly attached subscriber;
- `dialog.open`: one newly committed projection;
- `dialog.patch`: a contiguous revision update with positional operations and
  the full post-update interaction set;
- `dialog.replace`: one full projection when replacement is no larger than a
  patch;
- `dialog.close`: removal of one dialog id.

The dialog revision and patch rules reuse ADR-45's full-baseline, contiguous
revision, preflight-before-mutate, and full-interaction-set invariants. A full
replacement remains the fallback when a patch is not smaller. Dialog state is
committed before its event is broadcast under the same lock discipline as the
other surfaces. The initial SSE burst includes `dialog.snapshot` after the
Canvas and Timeline baselines, allowing reload and reconnect to converge even if
the opening event was missed.

### Browser lifecycle

The browser retains one root per projected dialog so field and attachment
snapshots cover visible and queued dialogs. Dialogs form a FIFO queue and only
the head is visible; nested stacked modals and competing focus traps are not
used. While the queue is non-empty, all non-dialog application content,
including the toast container, is inert. The visible dialog traps focus, Escape
...
does not optimistically remove the dialog. Only an accepted `dialog.close` event
removes it, avoiding client/server divergence after a failed request. Session
swap tears down all dialog roots, bindings, pending focus state, and inertness.
...
Calling Dialog a surface follows from its lifecycle, not its appearance. It is
...
- **Positive**: active dialogs and their server-committed values survive SSE reconnect and browser
  reload.
- **Positive**: Dialog reuses the established rendering, invoke, field,
...
- **Negative / trade-offs**: Canvas/Timeline-specific reachability and projection
  code must be generalized or extended to Dialog, increasing the initial change.
- **Negative / trade-offs**: `ui.dialog` is callback-based; a true cross-turn
  `await` requires a separate suspendable-runtime decision.
- **Negative / trade-offs**: only the queue head is presented, so later dialogs
