...
const inspect = (value, opts) => {
  ...
  function sanitize(val, d) {
    ...
    if (typeof Set !== "undefined" && val instanceof Set) {
      const items = [];
      val.forEach((v) => {
        items.push(sanitize(v, d1));
      });
      return items;
    }
    if (Array.isArray(val)) return val.map((v) => sanitize(v, d1));
    var out = {};
    Object.keys(val).forEach((k) => {
      out[k] = sanitize(val[k], d1);
    });
    return out;
  }
...
var util = Object.freeze({
  ...
  toJsArray: (value) => {
    if (typeof __utilToJsArray__ === "function")
      return __utilToJsArray__(value);
