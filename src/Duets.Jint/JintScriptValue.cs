...
internal sealed class JintScriptValue : ScriptValue
{
    ...
    {
        if (other is JintScriptValue jv)
            return this.Value.Equals(jv.Value);
        if (ReferenceEquals(other, Null))
            return this.Value.Type == Types.Null;
        if (ReferenceEquals(other, Undefined))
            return this.Value.Type == Types.Undefined;
        throw new InvalidOperationException("Cannot compare ScriptValues from different backends.");
    }

    protected override int GetHashCodeCore()
    {
        if (this.Value.Type == Types.Null)
            return 1;
        if (this.Value.Type == Types.Undefined)
            return 0;
